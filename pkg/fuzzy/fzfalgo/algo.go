// Extracted from github.com/junegunn/fzf v0.74.4 src/algo/algo.go (MIT,
...
func indexAt(index, length int, forward bool) int {
	if forward {
		return index
	}
...
func alloc16(offset int, slab *fzfutil.Slab, size int) (int, []int16) {
...
func alloc32(offset int, slab *fzfutil.Slab, size int) (int, []int32) {
...
func trySkip(input *fzfutil.Chars, caseSensitive bool, b byte, from int) int {
...
func runePrefilterable(input *fzfutil.Chars, pattern []rune, caseSensitive bool) bool {
	...
	for _, r := range pattern {
		...
		if r >= utf8.RuneSelf &&
			(unicode.ToUpper(r) != r || unicode.ToLower(r) != r || fzfutil.MayFoldToASCII(r)) {
			return false
		}
	}
	return true
...
func runeFuzzyIndex(input *fzfutil.Chars, pattern []rune, caseSensitive bool) (int, int) {
	runes := input.Runes()
	...
	for pidx := range pattern {
		...
		if last < utf8.RuneSelf {
			idx = indexASCIIRune(runes, caseSensitive, byte(last), idx) //nolint:gosec // last < utf8.RuneSelf
		} else {
...
func asciiFuzzyIndex(input *fzfutil.Chars, pattern []rune, caseSensitive bool) (int, int) {
	if !input.IsBytes() {
	...
	var b byte
	for pidx := range pattern {
		b = byte(pattern[pidx]) //nolint:gosec // isASCII(pattern) checked above
...
// allocates no arrays.
func fuzzyMatchV2Single(caseSensitive, forward bool, input *fzfutil.Chars, b byte, withPos bool) (Result, *[]int) {
...
func fuzzyMatchV2Two(caseSensitive, forward bool, input *fzfutil.Chars, pchar0, pchar1 byte, minIdx, maxIdx int, withPos bool, slab *fzfutil.Slab) (Result, *[]int) {
	sl := input.Bytes()
	N := maxIdx - minIdx
...
func FuzzyMatchV2(caseSensitive, normalize, forward bool, input *fzfutil.Chars, pattern []rune, withPos bool, slab *fzfutil.Slab) (Result, *[]int) {
	...
	if M == 1 && input.IsBytes() && pattern[0] < utf8.RuneSelf {
		return fuzzyMatchV2Single(caseSensitive, forward, input, byte(pattern[0]), withPos) //nolint:gosec // pattern[0] < utf8.RuneSelf
	}
	...
	if M == 2 && input.IsBytes() &&
		pattern[0] < utf8.RuneSelf && pattern[1] < utf8.RuneSelf {
		return fuzzyMatchV2Two(caseSensitive, forward, input, byte(pattern[0]), byte(pattern[1]), minIdx, maxIdx, withPos, slab) //nolint:gosec // both < utf8.RuneSelf
	}
...
func calculateScore(caseSensitive, normalize bool, text *fzfutil.Chars, pattern []rune, sidx, eidx int, withPos bool) (int, *[]int) {
