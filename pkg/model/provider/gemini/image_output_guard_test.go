...
// and only when an ordinary outgoing request also carries custom function
// tools or structured output. Server-side built-ins stay allowed.
func TestCheckImageOutputRequestCompatibility_GateConditions(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "title generation bypasses custom tools", apiSurface: apiSurfaceGateway, declared: declaredTrue, requestTools: 1, generatingTitle: true},
		{name: "compaction bypasses structured output", apiSurface: apiSurfaceGateway, declared: declaredTrue, structured: true, compacting: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.generatingTitle {
				client.ModelOptions = options.Apply(options.WithGeneratingTitle())
			}
			if tt.compacting {
				client.ModelOptions = options.Apply(options.WithCompacting())
			}
			...
			err := client.checkImageOutputRequestCompatibility(t.Context(), tt.declared != nil && tt.declared.Image != nil && *tt.declared.Image, config, tt.requestTools)
...
func TestImageOutputRequestIncompatibleError_ToolMessages(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want:              "image output is enabled for this model (by output_capabilities.image or models.dev) and is incompatible with tools in the same request; use a separate model or request for that combination",
		},
		{
			...
			toolCallSupport:   modelinfo.ToolCallUnsupported,
			want:              "image output is enabled for this model (by output_capabilities.image or models.dev) and is incompatible with structured output in the same request; use a separate model or request for that combination",
		},
	}
...
func TestCheckImageOutputRequestCompatibility_ToolCallSupport(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := client.checkImageOutputRequestCompatibility(t.Context(), true, &genai.GenerateContentConfig{}, 1)
...
func TestImageOutputRequestIncompatibleError_MessageNamesCategoriesOnly(t *testing.T) {
	...
	assert.Contains(t, msg, "models.dev")
	assert.NotContains(t, msg, "is configured for image output")
...
func TestImageOutputRequestIncompatibleError_RoutesThroughExistingErrorSeam(t *testing.T) {
	...
	err := &ImageOutputRequestIncompatibleError{Incompatibilities: []imageOutputIncompatibility{imageOutputIncompatibleTools}}
	...
	assert.Contains(t, visible, "tools")
