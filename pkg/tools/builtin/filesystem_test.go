...
func TestFindAndReplace(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "consecutive collapsible whitespace preserves surrounding text",
			content: "before\na  b\nafter",
			oldText: "a b",
			newText: "REPLACED",
			wantOK:  true,
		...
		{
			name:    "tab and space mix collapsed",
			content: "a \t \t b",
			oldText: "a b",
			newText: "X",
			wantOK:  true,
		...
		{
			name:    "escaped quotes in Dockerfile echo",
			content: `RUN echo \"hello\" && exit 1`,
			oldText: `RUN echo "hello" && exit 1`,
			newText: "REPLACED",
			wantOK:  true,
			want:    "REPLACED",
		},
		{
			name:    "unescaped quotes in LLM but file has escaped",
			content: `echo \"install failed\"; exit 1`,
			oldText: `echo "install failed"; exit 1`,
			newText: "REPLACED",
			wantOK:  true,
			want:    "REPLACED",
		},
		{
			name:    "line continuation with utf8 content",
			content: "RUN echo \\\n    \"héllo wörld\"",
			oldText: "RUN echo \"héllo wörld\"",
			newText: "REPLACED",
			wantOK:  true,
			want:    "REPLACED",
		},
		{
			name:    "multiple continuations with surrounding content preserved",
			content: "FROM node\n\nRUN apt-get update \\\n    && apt-get install -y \\\n    curl \\\n    wget\n\nCMD [\"node\"]",
			oldText: "RUN apt-get update && apt-get install -y curl wget",
			newText: "RUN apt-get update && apt-get install -y curl wget git",
			wantOK:  true,
			want:    "FROM node\n\nRUN apt-get update && apt-get install -y curl wget git\n\nCMD [\"node\"]",
		},
		{
			name:    "indentation mismatch with utf8",
			content: "  说明：テスト\n  次の行",
			oldText: "说明：テスト\n次の行",
			newText: "REPLACED",
			wantOK:  true,
		...
		{
			name:    "trailing whitespace with utf8",
			content: "héllo   \nwörld",
			oldText: "héllo\nwörld",
			newText: "REPLACED",
			wantOK:  true,
		...
		{
			name:    "whitespace before line continuation",
			content: "RUN cmd1   \\\n   && cmd2",
			oldText: "RUN cmd1 && cmd2",
			newText: "REPLACED",
			wantOK:  true,
			want:    "REPLACED",
		},
