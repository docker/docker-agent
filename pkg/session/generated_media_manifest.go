...
type GeneratedFile struct {
	...
	// RelPath is the exact workspace-relative, slash-separated path written.
	RelPath string

	// Root is the artifact root kind the path is interpreted against:
	// chat.ArtifactRootWorkspace (the default — an empty value is normalized
	// to it). Resolution must require it to match the reference's root.
...
// generated-media artifact reference: a path may only be read back if the (owner
...
// so callers omitting the root and legacy rows keep their workspace meaning.
...
func validateGeneratedFileRecord(sessionID string, root chat.ArtifactRootKind, relPath string) error {
	...
	if normalizeGeneratedFileRoot(root) != chat.ArtifactRootWorkspace {
...
// write and lookup. A key has the workspace-relative shape
// workspacemedia.Write guarantees; anything else — traversal, NUL,
