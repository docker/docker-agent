...
func TestResolveWorkingDir_OwnRoot(t *testing.T) {
	t.Parallel()
	sess := &Session{ID: "s1", WorkingDir: "/work/project"}
...
func TestResolveWorkingDir_RootNeedNotExist(t *testing.T) {
	...
	sess := &Session{ID: "s1", WorkingDir: "/definitely/gone/workspace"}

	got, err := ResolveWorkingDir(t.Context(), sess, nil)
	require.NoError(t, err)
	assert.Equal(t, "/definitely/gone/workspace", got)
}

func TestResolveWorkingDir_InheritsFromParentChain(t *testing.T) {
	t.Parallel()
	root := &Session{ID: "root", WorkingDir: "/work/a"}
...
func TestResolveWorkingDir_DepthBound(t *testing.T) {
	...
	t.Run("exactly at bound succeeds", func(t *testing.T) {
		t.Parallel()
		leaf, lookup := chainOfDepth(maxWorkingDirAncestry, "/work/deep")
		got, err := ResolveWorkingDir(t.Context(), leaf, lookup)
		require.NoError(t, err)
		assert.Equal(t, "/work/deep", got)
	})

	t.Run("one beyond bound fails", func(t *testing.T) {
		t.Parallel()
		leaf, lookup := chainOfDepth(maxWorkingDirAncestry+1, "/work/deep")
...
func TestSubSessionWorkingDirRoundTrip(t *testing.T) {
	...
	for name, newStore := range stores {
		t.Run(name, func(t *testing.T) {
			...
			parent := New(WithWorkingDir("/work/project"))
			...
			assert.Equal(t, "/work/project", reloaded.WorkingDir)
			...
			require.NoError(t, err)
			assert.Equal(t, "/work/project", resolved)
		})
	}
}
