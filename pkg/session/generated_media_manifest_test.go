...
// TestGeneratedMediaManifest_WorkspaceRootNormalized verifies empty root
// kinds retain their workspace meaning on read-back.
...
func TestGeneratedMediaManifest_RejectsMisRootedRecords(t *testing.T) {
	...
	manifestStores(t, func(t *testing.T, _ Store, manifest GeneratedMediaManifest) {
		...
		require.ErrorIs(t, add("external", "cat.png"), ErrInvalidGeneratedFileRoot)
		require.ErrorIs(t, add("attacker-root", "cat.png"), ErrInvalidGeneratedFileRoot)

		_, err := manifest.LookupGeneratedFile(t.Context(), "owner", absolutePath)
		require.ErrorIs(t, err, ErrInvalidGeneratedFilePath)
	})
}
