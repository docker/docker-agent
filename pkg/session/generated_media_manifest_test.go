...
func TestGeneratedMediaManifest_ExternalRoundTrip(t *testing.T) {
	...
	manifestStores(t, func(t *testing.T, _ Store, manifest GeneratedMediaManifest) {
		...
		require.NoError(t, manifest.AddGeneratedFile(t.Context(), GeneratedFile{
			SessionID: "owner",
			RelPath:   "/tmp/exports/cat.png",
			Root:      chat.ArtifactRootExternal,
			MimeType:  "image/png",
			CreatedAt: time.Now(),
		}))

		got, err := manifest.LookupGeneratedFile(t.Context(), "owner", "/tmp/exports/cat.png")
		require.NoError(t, err)
		assert.Equal(t, chat.ArtifactRootExternal, got.Root)
		assert.Equal(t, "/tmp/exports/cat.png", got.RelPath)
	})
}
...
func TestGeneratedMediaManifest_RejectsMisRootedRecords(t *testing.T) {
	...
	manifestStores(t, func(t *testing.T, _ Store, manifest GeneratedMediaManifest) {
		...
		require.ErrorIs(t, add("", "/etc/passwd"), ErrInvalidGeneratedFilePath,
			"a workspace record must never carry an absolute path")
		require.ErrorIs(t, add(chat.ArtifactRootWorkspace, "/abs/cat.png"), ErrInvalidGeneratedFilePath)
		...
		require.ErrorIs(t, add(chat.ArtifactRootExternal, "/abs/../cat.png"), ErrInvalidGeneratedFilePath,
			"an external record must carry a clean path")
		require.ErrorIs(t, add("attacker-root", "/abs/cat.png"), ErrInvalidGeneratedFileRoot)

		for _, p := range []string{"/etc/passwd", "/abs/cat.png"} {
			_, err := manifest.LookupGeneratedFile(t.Context(), "owner", p)
			require.ErrorIs(t, err, ErrGeneratedFileNotFound, "unrecorded absolute path %q must be refused", p)
		}
	})
}
