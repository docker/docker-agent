...
// Image-output models are eligible title candidates because Gemini title requests
...
func TestLocalRuntime_TitleGenerator_AllImageOutputCandidatesRemainEligible(t *testing.T) {
	...
	assert.NotNil(t, rt.TitleGenerator(t.Context()),
		"image-output models remain eligible for title generation")
...
// title requests omit image modalities, and failure falls through to the agent model.
...
// image-output model remains usable by both normal and title calls.
