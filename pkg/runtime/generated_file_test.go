...
func TestResolveGeneratedFile_OversizedFileRefused(t *testing.T) {
	t.Parallel()
	sess, root := workspaceSession(t, "sess-oversized")
	r, store := resolverTestRuntime(t, sess)
	recordWorkspaceFile(t, store, sess.ID, root, "cat.png", []byte("png"))
	require.NoError(t, os.Truncate(filepath.Join(root, "cat.png"), maxGeneratedFileBytes+1))
