...
func TestMaterializeGeneratedMedia_NoWorkspaceRoot(t *testing.T) {
	...
	warnings := sink.warnings()
	require.Len(t, warnings, 2, "every failed blob remains visible to the caller")
	...
	assert.NotEqual(t, warnings[0].Message, warnings[1].Message, "an all-failed batch must retain each item's oracle")
