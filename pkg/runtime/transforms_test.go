...
func TestRunStream_FallbackUsesItsOwnCapabilities(t *testing.T) {
	t.Parallel()

	primary := &failingProvider{id: "custom/text-only", err: errors.New("bad request")}
	fallback := &recordingMsgProvider{mockProvider: mockProvider{
		id:     "dmr/ai/qwen2.5-vl",
		stream: newStreamBuilder().AddContent("ok").AddStopWithUsage(1, 1).Build(),
	}}
	fallback.baseConfig = base.Config{ModelConfig: latest.ModelConfig{
		Provider: "dmr",
	...
	tm := team.New(team.WithAgents(a))
	r, err := NewLocalRuntime(t.Context(), tm, WithSessionCompaction(false), WithModelStore(modalityModelStore{}))
	require.NoError(t, err)
	...
	assert.True(t, hasImagePart(fallback.got[0]), "fallback transforms must use the fallback provider's capabilities")
}

func hasImagePart(messages []chat.Message) bool {
	for _, message := range messages {
		for _, part := range message.MultiContent {
			if part.Type == chat.MessagePartTypeImageURL || part.Document != nil && strings.HasPrefix(part.Document.MimeType, "image/") {
				return true
			}
		}
	}
