...
// replacement, missing or oversized file — collapses into it so UIs can
...
// that the path still names this exact regular, size-bounded file rather
// than a symlink swapped in since materialization.
func readRegularGeneratedFile(f *os.File, lstat func() (os.FileInfo, error)) ([]byte, error) {
	...
	if st.Size() > maxGeneratedFileBytes {
		return nil, fmt.Errorf("recorded file is %d bytes, above the %d-byte display bound", st.Size(), maxGeneratedFileBytes)
	}
	lfi, err := lstat()
	if err != nil {
	...
	if len(data) > maxGeneratedFileBytes {
		return nil, errors.New("recorded file grew above the display bound during read")
	}
	return data, nil
}
