...
for exact configuration and limitations — requests with tools or structured
output are rejected locally before any request is sent on every supported
Google surface. For custom tools, models.dev metadata lets the error distinguish
...
file, the same way generated code or text lands there. A portable copy is
also stored in the session database. Generated files remain regular
untracked workspace files: visible to every tool, and yours to edit,
commit, move, or delete. On a remote runtime the files are written into that
...
the TUI prints the saved workspace path instead. Reopening a session reads
the manifest-gated portable database copy first, so generated images
survive workspace edits, deletion, and session moves. Generated-media
resolution does not impose a size cap on that stored copy. Sessions created
before portable copies were stored fall back to their manifest-gated
workspace files without a generated-media size cap; if such a file was
deleted, moved, replaced by a symlink, became invalid, or cannot be read,
the transcript shows a short "unavailable" note rather than wrong content.
